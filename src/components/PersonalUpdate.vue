<template>
  ...
  <b-modal 
     ...
     <!-- 修改密码成功失败的提示 的模态框 Start-->
       ...
       >
        ...
        <div class="d-block text-center">
         ...
         <div slot="modal-footer" class="w-100 text-center">
           <b-btn size="md" type="button" @click="isSuccessModal=false"
            variant="danger">好的
           </b-btn>
        </div>
       </b-modal>
     ...
     <b-modal
          ...
          no-close-on-backdrop
          body-class="text-center"
          v-model="selectTipModal"
       >
        <div slot="modal-header" class="w-100">
           <span class="float-left">提示</span>
           <span class="float-right pointer">
            <img src="../assets/images/esc.png" class="esc-img" 
                @click="selectTipModal=false">
           </span>
        </div>
        <div class="d-block text-center">
          <h4>至少要选择一个关注类型！</h4>
         ...
         <div slot="modal-footer" class="w-100 text-center">
           <b-btn size="md" type="button" @click="selectTipModal=false"
            variant="danger">好的
           </b-btn>
        </div>
       </b-modal>            
  ...
  export default{
          ...
          var token =this.token;
           self.$ajax({
            ...
            dataType: 'json'
              ...
              self.email = person.email;
              self.profile = "https://recruitment.cqcyit.com:8090/users_management/getOriginalProfilePhoto/"+person.id;
              //赋值
              ...
              self.person.profile = "https://recruitment.cqcyit.com:8090/users_management/getOriginalProfilePhoto/"+person.id;
    ...
    methods:{
        ...
        hidenUpdataImg:function(){
          ...
          this.$refs.cropper.replace(this.profile);
        },
         cropImage () {
        ...
        if(!passwordReg.test(this.password)){
          ...
          var token = this.token;
            ...
            dataType: 'json'
              ...
              if(data.data.state == "false") {
                this.error=true;
                this.$refs.errMsg.innerHTML = "密码错误，请重新输入！"
              } else {
              this.error=false;
                    }
          }.bind(this))

      ...
      <img src="../assets/images/logo.png" class="logo" @click="toSearch"  @mouseover="toPointer($event)"/>
          ...
          <b-button class="search-btn search-btn-sm" type="button" @click="search(0)">搜索</b-button> 
            <b-nav>
              ...
              <li>
                <a  v-if="role==1" @mouseover="toPointer($event)" @click="toAdmin">后台管理</a>
              </li>
            </b-nav>
        ...
        <li>
              <div class="logo per-ico pointer"><img src="../assets/images/person.png" ></div>
                 <ul  class="header-select per-header-select" >
                  <li v-for="" style="float: left;background-color: white;"><a class="pername text-center" @mouseover="toPointer($event)" @click="toPersonal">{{name}}</a></li>

                  <li style="margin-left: 22%;font-size: 5px;"><b-btn style="font-size: 5px;"variant="danger" class="text-center"@click="toLogin" >退出登录</b-btn></li>
                 </ul>   
            </li>
              ...
              <div style=""><img src="../assets/images/notifications.png" style="margin-top: 20px" class="logo lingdang-ico pointer">
                <span class="dot">{{msgNum}}</span>
                  ...
                  <ul  class="header-select msg-ul" v-if="msgNum!=0">
                   ...
                   <li ><b-btn class="pointer msgMore"variant="danger"  @mouseover="toPointer($event)" @click="toPersonalMes">查看更多</b-btn></li>
                  </ul>
            </li>


            </ul>
            <!-- <Headermodal @transferKeyword="getKeyword"></Headermodal> -->
        ...
        <div class="bid-info " v-for="result in results">
          <div class="pub-bid text-left">
            <div style="display:inline-block;margin-top: 10px">
                  ...
                  <!--  <b-list-group-item  class=" tab-sort tag pointer red pubBid"id="pubBid" v-for="pubBid in pubBids" @click="searchPubBid(pubBid)"><span>{{pubBid}}</span></b-list-group-item> -->
          ...
          <div class="bid">
                <h5   @mouseover="toPointer($event)" @click="browseRecord(result._source.id)" v-show="result.highlightTitle==false" class="text-left pointer" style="margin-left:6px;color:#e64035">
                  <a class="showtwo" target="_blank" style="color:#e64035" v-html="result._source.tender_title">{{result._source.tender_title}}</a><span style="margin-right:12px;margin-top: 6px; font-size:14px;float: right;">{{result._source.tender_issue_time}}</span>
                </h5>
                ...
                <a  @mouseover="toPointer($event)" @click="browseRecord(result._source.id)" v-show="result.highlightContent==true" target="_blank" v-bind:href="result._source.tender_content_url"><p class="re-content text-left" v-html="result.highlight.tender_content">
                </p></a>
          </div>
          <div class="bid-foot text-right">
            <span class="bid-btn focus  " >
              关注量
            </span>
            ...
            <span class="feedback text-align pointer bid-btn focus " >浏览量
            <span class="tab-title  followTotal" style="padding-right: 2px;padding-left: 2px ;width: 20px;margin-top: 0px">{{result._source.browseCount}}</span></span>
          </div>
        </div>
        <!-- 分页 -->
        <div>
           <el-pagination v-if="currentTotal!=0" @current-change="handleCurrentChange" background layout="prev, pager, next" :total="currentTotal" class="currentTotal":current-page.sync="currentPage">
             </el-pagination>
        </div>
...
<style scoped>
  ...
  /*.pername:hover{background-color: white;}*/
...
/*-moz-box-shadow: 0 1px 3px rgba(0,0,0,1); -o-box-shadow: 0 1px 3px rgba(0,0,0,1); box-shadow: 0 1px 3px rgba(0,0,0,1); -webkit-box-shadow: 0 1px 3px rgba(0,0,0,1);*/
...
<script >
  ...
  import {loading} from '../assets/js/base-loading.js'
  export default{
    data:function(){
      return{
        form:{
    ...
    created:function(){
      ...
      if (self.$route.query.keyword!='undefined') {
      ...
      console.log(self.form.keyword);
          ...
          for (i;i<places.length;i++) {
            // console.log("重庆市万州去");
            // console.log(places[4].fields.tender_area.length);
...
//            })
    ...
    methods:{
      ...
      storage.clear();
          ...
          // this.$router.push({name:'Login',params:{usermsg:this.usermsg}});
        },
      ...
      window.open(routeData.href, '_blank');
        ...
        clearConditions:function(){
          ...
          // $('.tab-item  .msg-type .tab-sort .type').eq(0).trigger("click","true");
          ...
          $('.place-wrap .tab-sort').eq(0).trigger("click","true");
          // $('.my-qualification .qualification .tab-sort').eq(0).trigger("click","true");
          ...
          // $("#pub-select").val("");
          // $("#pub-select").trigger("change","true");

          // $("#major-select").val("");
          // $("#major-select").trigger("change","true");

          // $("#cert-select").val("");
          // $("#cert-select").trigger("change","true");
        ...
        searchHot:function(from){
          ...
          this.$ajax({
            ...
            console.log("response");
            console.log(response);
            var results = response.data.data.hits.hits;
        ...
        // toPointers : function ($event) {
        //    $event.target.classList.add("over-pointer");
        //    $event.target.classList.add("tab-choosed");
        // },
      ...
      window.open(routeData.href, '_blank');
        // this.$router.push({path:'/announce',query:{token:this.token,id:id}});
        // console.log(this.results._source.id);
          this.$ajax({
            method: 'post',
